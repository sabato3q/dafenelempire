...
function money(v) {
  return `GH₵${Number(v || 0).toFixed(0)}`;
}
...
async function loadStats() {
  ...
  $('productCount').textContent = pc || 0;
  $('orderCount').textContent = oc || 0;
  $('testimonialCount').textContent = tc || 0;
}
...
async function loadProducts() {
  ...
  $('productList').innerHTML = (data || []).map(p => `
    <div class="product-row">
      <img
        src="${esc(p.image_url || 'herbal-detox-tea.jpg')}"
        alt="${esc(p.name)}"
...
async function loadOrders() {
  ...
  $('orderList').innerHTML = (data || []).map(o => `
    <div class="order-row">
      <div>
        <strong>${esc(o.customer_name || 'Customer')}</strong><br>
        ${esc(o.customer_phone || 'No phone')}<br>
      ...
      <div>
        ...
        <pre>${esc(JSON.stringify(o.items, null, 2))}</pre>
      </div>
    </div>
...
async function loadTestimonials() {
  ...
  $('testimonialList').innerHTML = (data || []).map(t => `
    <div class="testimonial-row">
      <span>
        <b>${esc(t.customer_name)}</b><br>
        ${esc(t.message)}<br>
        <small>${t.approved ? 'Approved' : 'Hidden'}</small>
      </span>
...
window.editProduct = async id => {
  ...
  $('productAvailable').checked = data.available;

  $('editorTitle').textContent = 'Edit Product';

  showPanel('productEditor');
};
...
window.toggleTestimonial = async (id, approved) => {
  await db
    .from('testimonials')
    .update({ approved })
    .eq('id', id);
...
$('cancelEdit').onclick = () => showPanel('products');
...
$('productForm').onsubmit = async e => {
  ...
  const payload = {
    ...
    image_url:
      $('productImage').value.trim() || 'herbal-detox-tea.jpg',
...
$('testimonialForm').onsubmit = async e => {
  ...
  const result = await db
    ...
    .insert({
      ...
      message: $('testimonialText').value.trim(),
      approved: true
    });
